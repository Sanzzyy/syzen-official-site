...
export default function Pricing() {
  ...
  // Data tetap sama persis seperti sebelumnya
  const pricingData = {
    "Company Profile": [
      {
        name: "Basic",
  ...
  return (
    <section id="pricing" className="py-24 bg-black text-white">
      <div className="max-w-6xl mx-auto px-6">
        ...
        <div
          key={activeTab}
          className={`grid gap-6 md:gap-8 items-stretch animate-fade-in-up mx-auto ${
        ...
        >
          {pricingData[activeTab].map((plan, index) => (
            <div
              key={index}
              // Padding dikurangi jadi p-6 md:p-8 biar card lebih padat dan gak kepanjangan
              className={`relative flex flex-col p-6 md:p-8 rounded-3xl transition-all duration-500 ${
                plan.isPopular
                  ? "bg-white dark:bg-zinc-800 text-black dark:text-white shadow-2xl z-10 border-4 border-white dark:border-zinc-700 md:scale-105"
            ...
            >
              ...
              <div className="mb-6 flex flex-col flex-shrink-0">
                <div className="flex flex-wrap items-center gap-3 mb-1">
                  <div className="bg-red-500/20 border border-dashed border-red-500 text-red-500 px-2 py-1 rounded-md text-[10px] sm:text-xs font-bold tracking-wider">
                    🎟️ PROMO
                  </div>
                  <span
                    className={`text-sm md:text-base font-semibold line-through ${
                      plan.isPopular
              ...
              <div
                className={`w-full h-px mb-6 ${plan.isPopular ? "bg-gray-200 dark:bg-zinc-700" : "bg-zinc-800"}`}
              ></div>

              {/* 4. Fitur-fitur (Ditaruh di bawah tombol) */}
              <div className="flex-grow">
                <span
                  className={`text-xs font-bold uppercase tracking-wider block ${plan.isPopular ? "text-gray-500" : "text-gray-400"}`}
                >
                  Yang Anda Dapatkan:
                </span>
                ...
                {/* 3. TOMBOL DIPINDAH KE ATAS (Sesuai gaya Claude) */}
                <a
                  ...
                  className={`block w-full text-center py-3 md:py-4 rounded-xl font-bold text-base md:text-lg transition-all transform hover:-translate-y-1 flex-shrink-0 mb-6 ${
                    plan.isPopular
